...
**Sequential numbering within epics.** Stories under an epic carry a sequential `s<N>-` prefix in their `id` and filename, where `<N>` is the story's position in the epic's `children:` list (zero-indexed). Example: `s0-story-cc-audit`, `s1-story-cc-pin-dep`. The prefix makes ordering visible at a glance and stable in `ls` output. Inserting a story mid-list is a deliberate re-numbering operation — the epic's `children:` list and any cross-references update together. Tasks under a story may follow the same `t<N>-<slug>` convention.
